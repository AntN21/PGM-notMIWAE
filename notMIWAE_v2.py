...
class notMIWAE_v2:
    ...
    def __init__(self, X, Xval,
        ...
        if permutation_invariance:
            svars = tf.compat.v1.trainable_variables('data_process')
            svars.append(self.global_step)
            self.saver = tf.compat.v1.train.Saver(svars)
        else:
            self.saver = tf.compat.v1.train.Saver()

        tf.compat.v1.summary.scalar('Evaluation/loss', self.loss)
        tf.compat.v1.summary.scalar('Evaluation/pxz', tf.reduce_mean(self.log_p_x_given_z))
        tf.compat.v1.summary.scalar('Evaluation/psx', tf.reduce_mean(self.log_p_s_given_x))
        tf.compat.v1.summary.scalar('Evaluation/qzx', tf.reduce_mean(self.log_q_z_given_x))
        tf.compat.v1.summary.scalar('Evaluation/pz', tf.reduce_mean(self.log_p_z))
    ...
    def bernoulli_decoder_miss(self, z):
        ...
        if self.missing_process == 'selfmasking':
            ...
            self.W = tf.compat.v1.get_variable('W', shape=[1, 1, self.d])
            self.b = tf.compat.v1.get_variable('b', shape=[1, 1, self.d])
    ...
    def get_ELBO(self, q_z, lpxz):
        ...
        self.KL = self.KL_loss(q_z.loc, tf.math.log(tf.square(q_z.scale)))
        # ---- compare manual KL loss to tf.distributions
        p_z = tf.compat.v1.distributions.Normal(loc=0.0, scale=1.0)
        self.KL_check = tf.reduce_sum(tf.compat.v1.distributions.kl_divergence(q_z, p_z), axis=1)
    ...
    def get_notMIWAE(self, lpxz, lpmz, lqzx, lpz):
        ...
        log_avg_weight = log_sum_w - tf.math.log(tf.cast(self.n_pl, tf.float32))
    ...
    def bernoulli_loss(x, s, y):
        eps = np.finfo(float).eps
        p_x_given_z = x * tf.math.log(y + eps) + (1 - x) * tf.math.log(1 - y + eps)
